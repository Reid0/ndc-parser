  ...
  this.parseDataCommand = function(splitted){
    ...
    case '1':
      ...
      case '2':
        ...
        for(let i = 4; i < splitted.length; i++){
          ...
          } else { // the last field might be mac data if message authentication is used in the communicaiton 
            if(splitted[i].length === 8){ //check the lenght of the last element to identify if it is mac data or a state 
              parsed.mac_data = splitted[i]; //todo: handle mac_data only if MAC support is enabled on ATM level and only for the specified message_subclassparsed.mac_data = splitted[splitted.length - 1]; //todo: handle mac_data only if MAC support is enabled on ATM level and only for the specified message_subclass
            } else {
              parsed.states.push(splitted[i]);
            }
          }
      ...
      case '6':
        ...
        break;
      case 'E':
        parsed.message_identifier = 'Dispenser Currency Cassette Mapping Table';
        parsed.cassette_mapping_table = splitted[4];
        break;
      case 'A':
        parsed.message_identifier = 'Enhanced Configuration Data';
        parsed.options_table = splitted[4];
        parsed.timers_table = splitted[5];
        break;
    ...
    case '4':
      ...
      case '2':
        ...
        break;
      case 'H':
        parsed.modifier = 'Send all KVVs';
        break;
      case '5':
        parsed.modifier = 'Decipher new MAC key with current master key';
        parsed.new_key_length = splitted[4].substr(0, 3);
        parsed.new_key_data = splitted[4].substr(3);
        break;        
      }
